...
export async function runAgentLoop(params: {
  ...
  /** 压缩检查：超阈值时压缩并同步回原数组；返回是否执行了压缩 */
  async function maybeCompact(context: string): Promise<boolean> {
    if (!needsCompaction(messages)) return false;
    const compacted = await compactHistory({ messages, config, onLog });
    messages.length = 0;
    messages.push(...compacted);
    log("info", `压缩检查（${context}）：已压缩，当前 ${estimateTokens(messages)} tokens 估算`);
    return true;
  }

  // 压缩检查：每次运行前
  await maybeCompact("运行前");
  ...
  for (let i = 0; i < MAX_LOOP_ITERATIONS; i++) {
    ...
    if (assistant.tool_calls && assistant.tool_calls.length > 0) {
      ...
      // 循环内复查：工具结果可能已把 token 推过阈值，超了就在下次调模型前压缩
      await maybeCompact(`循环内第 ${i + 1} 轮后`);
